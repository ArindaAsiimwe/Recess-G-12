...
public class Server {
    ...
    private static String dbUrl = "jdbc:postgresql://localhost:5432/uprisedb";
        ...
        try (Connection connection = DriverManager.getConnection(dbUrl, dbUsername, dbPassword)) {
            ...
            if (checkReceiptResult.next()) {
                ...
                if (count > 0 && "pending".equals(receiptStatus)) {
                    ...
                    String insertSql = "INSERT INTO transactions (user_id, amount, date_deposited, receipt_number, status) " +
                            "SELECT c.membernumber, ?, TO_DATE(?, 'YYYY-MM-DD'), ?, 'success' " +
                            "FROM clients c " +
                            "WHERE c.username = ?";
                    PreparedStatement insertStatement = connection.prepareStatement(insertSql);
                    insertStatement.setBigDecimal(1, new BigDecimal(amount));
                    insertStatement.setString(2, dateDeposited);
                    insertStatement.setString(3, receiptNumber);
                    insertStatement.setString(4, username);
                    int rowsAffected = insertStatement.executeUpdate();
    
                    if (rowsAffected > 0) {
                        // Update the status of the receiptNumber to "invalid" in the BankReceipts table
                        String updateReceiptSql = "UPDATE BankReceipts SET status = 'invalid' WHERE receiptnumbers = ?";
                        PreparedStatement updateReceiptStatement = connection.prepareStatement(updateReceiptSql);
                        updateReceiptStatement.setString(1, receiptNumber);
                    ...
                    return "Error: Receipt number is not available or already used.";
                }
            } else {
                return "Error: Receipt number not found.";
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private static String checkStatement(String dateFrom, String dateTo, String username) {
        try (Connection connection = DriverManager.getConnection(dbUrl, dbUsername, dbPassword)) {
            ...
            while (resultSet.next()) {
                ...
                statementData.append(String.format("Transaction ID: %s\tAmount: %-10.2f\tDate Deposited: %-15s\tReceipt Number: %-20s\t",
    ...
    // Insert loan status, contribution status, and account balance at the end of statementData
    int lastIndex = statementData.length();
    statementData.insert(lastIndex, String.format("\tAccount Balance: %-10.2f", accountBalance));
    statementData.insert(lastIndex, String.format("\tContribution Status: %-10s", getContributionStatus(contributionProgress, totalLoanMonthsString)));
    ...
    // private static double calculatePerformance(double loanProgress, double contributionProgress) {
    ...
    private static int handleRegisterCommand(String command, PrintWriter writer, BufferedReader reader) throws IOException {
        String[] parts = command.split(" ");
        if (parts.length == 5) {
            ...
            if (membernumber != 0) {
                writer.println("Registration successful! Your membernumber is: " + membernumber + ". Please enter 'login' to login and continue.");
    ...
    private static boolean isUsernameTaken(String username) {
        try (Connection connection = DriverManager.getConnection(dbUrl, dbUsername, dbPassword)) {
            ...
            e.printStackTrace();
            return true; // In case of an error, assume the username is taken to avoid potential registration issues.
        }
    }
    
    private static int registerUser(String username, String password, String email, String telephone) {
        try (Connection connection = DriverManager.getConnection(dbUrl, dbUsername, dbPassword)) {
            String insertSql = "INSERT INTO clients (username, password, email, telephone) VALUES (?, ?, ?, ?) RETURNING membernumber";
            PreparedStatement insertStatement = connection.prepareStatement(insertSql);
    ...
    private static void handleLoanAcceptCommand(String command, PrintWriter writer, String username) {
        ...
        String loanApplicationId = parts[1];
        // Assuming you have the method to approve the loan application by loan application ID and equitable amount
    ...
    private static void handleLoanRejectCommand(String command, PrintWriter writer, String username) {
        ...
        String loanApplicationId = parts[1];
        // Assuming you have the method to reject the loan application by loan application ID
    ...
    private static String acceptLoanApplication(String loanApplicationId, double equitableAmount) {
        try (Connection connection = DriverManager.getConnection(dbUrl, dbUsername, dbPassword)) {
            // First, update the loan application status and equitable amount in loanrequests table
